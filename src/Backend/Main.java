...
class MatchService {
    ...
    public static Predicate<User> match(User currentUser) {

        return otherUser ->
                otherUser.skillsOffered.stream()
                        .anyMatch(skill -> currentUser.skillsWanted.contains(skill));
    }
}
...
public class Main {
    public static void main(String[] args) {
        ...
        users.stream()
                .filter(user -> user != a)
                .filter(MatchService.match(a))
                .forEach(user ->
                        System.out.println(a.name + " matches with " + user.name)
                );
    }
}
